...
6. **（关键步骤）进入基础报表后，对数据明细区域的「3条横线图标」（从右往左第2个按钮）执行 hover 悬浮3秒（不要点击！），出现下拉后点击「日报」选项，等待日报视图激活**
7. 确认合计行数据出现
8. 检查数据逻辑（消费=展现量×点击率等）
9. 用 lark-cli 写入飞书表格（创建 .bat 文件执行）
10. 验证 revision 号变化确认写入成功
11. 如执行失败：重试1次，仍失败则记录错误并上报
